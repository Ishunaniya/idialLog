...
make                     # x64: build/x64/dialLog_v1.10.7.exe
...
```bash
make windows-x86         # build/x86/dialLog_v1.10.7.exe
```
...
| 处 | 表现 |
|---|---|
| **exe 文件名** | `dialLog_v1.10.7.exe`(Makefile 从 `version.h` 解析) |
| exe 版本资源 | 右键→属性→详细信息:`FileVersion` / `OriginalFilename` |
| 标题栏 | `dialLog v1.10.7 — 拨号日志分析` |
