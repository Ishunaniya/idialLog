...
- **结论 ★**(核心):自动根因 + 处置建议 + **每条结论的日志证据(行号/时间戳)**。
  ...
  **无证据支撑的结论一律不输出**(宁可少说,不臆测)。
  ...
  当前已定位行。导出菜单可生成含摘要、多日志对比、小区统计、结论和人工书签的 Markdown 报告。
...
make ui-smoke    # wine+xvfb 启动真实 exe，验证后台加载、8 页导航、18 列指标与搜索历史
```
...
| `src/win32/load_controller.*` | 后台打开/取消、拖拽、粘贴、分析刷新及 CSV/Markdown 导出流程 |
