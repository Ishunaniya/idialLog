...
make                     # x64: build/x64/dialLog_v1.10.6.exe
...
```bash
make windows-x86         # build/x86/dialLog_v1.10.6.exe
```
...
统计虚拟时间线/指标表省去的预写单元格数,以及 1920px 信号图降采样后的绘制点数;
不设置依赖机器负载的墙钟阈值。
...
| 处 | 表现 |
|---|---|
| **exe 文件名** | `dialLog_v1.10.6.exe`(Makefile 从 `version.h` 解析) |
| exe 版本资源 | 右键→属性→详细信息:`FileVersion` / `OriginalFilename` |
| 标题栏 | `dialLog v1.10.6 — 拨号日志分析` |
